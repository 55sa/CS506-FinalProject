"""Optuna hyperparameter tuning for XGBoost regression (CPU)."""
...
def save_results(best_mae: float, best_params: dict, args: argparse.Namespace) -> None:
    ...
    output_path = output_dir / "xgboost_tuning.json"
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Tune XGBoost with Optuna (MAE, CPU)")
...
def main() -> None:
    args = parse_args()
    logger.info("=" * 80)
    logger.info("XGBoost Hyperparameter Tuning (MAE, 3-fold CV, CPU)")
