...
def calculate_request_types_overall(df: pd.DataFrame, top_n: int = 10) -> pd.Series:
    ...
    logger.info(f"Most common: {type_counts.index[0]} ({type_counts.iloc[0]:,})")
    return type_counts


def calculate_top_neighborhoods(df: pd.DataFrame, top_n: int = 10) -> pd.Series:
...
def calculate_source_distribution(df: pd.DataFrame) -> pd.Series:
    ...
    logger.info(
        f"{len(source_counts)} sources found, " f"top: {source_counts.index[0]} ({top_pct:.1f}%)"
    )
...
def calculate_trends_by_reason(df: pd.DataFrame, top_n: int = 8) -> pd.DataFrame:
    ...
    trends = (
        df[df["reason"].isin(top_reasons)].groupby(["year", "reason"]).size().unstack(fill_value=0)
    )
    logger.info(f"Calculated trends for {len(trends)} years")
    return trends
...
def calculate_trends_by_queue(df: pd.DataFrame, top_n: int = 5) -> pd.DataFrame:
    ...
    trends = (
        df[df["queue"].isin(top_queues)].groupby(["year", "queue"]).size().unstack(fill_value=0)
    )
    logger.info(f"Calculated trends for {len(trends)} years")
    return trends
...
def calculate_request_types_by_neighborhood_data(
    ...
    """
    logger.info(f"Calculating top {top_types_per_hood} types " f"for {top_hoods} neighborhoods")
...
def calculate_top_types_by_year(df: pd.DataFrame, top_n: int = 5) -> pd.DataFrame:
    ...
    trends = df[df["type"].isin(top_types)].groupby(["year", "type"]).size().unstack(fill_value=0)
    logger.info(f"Calculated trends for {len(trends)} years")
    return trends
